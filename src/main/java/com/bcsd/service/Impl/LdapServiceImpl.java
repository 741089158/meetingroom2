...
public class LdapServiceImpl implements LdapService {
    ...
    //    private static String searchFilter = "(&(objectClass=User)(name=zhangsan))"; //specify the LDAP search filter
//    private static String searchBase = "OU=北辰时代,DC=lin,DC=com"; //Specify the Base for the search//搜索域节点
    ...
    public Map<String, String> queryUser(String name) {
        ...
        try {
            ...
            String searchBase = "OU=Users Org Chart,DC=lin,DC=com";
            String returnedAtts[] = {
                    "url",  "name", "physicalDeliveryOfficeName",  "telephoneNumber",
                    "homePhone", "mobile", "department", "sAMAccountName","mail"}; //定制返回属性
            searchCtls.setReturningAttributes(returnedAtts); //设置返回属性集
            NamingEnumeration answer = ctx.search(searchBase, searchFilter, searchCtls);
            while (answer.hasMoreElements()) {
                SearchResult sr = (SearchResult) answer.next();
    ...
    /*public static void main(String args[]) {
        ...
        try {
            ...
            String searchFilter = "(&(objectClass=User)(name=张三))";
            String searchBase = "OU=北辰时代,DC=lin,DC=com"; //Specify the Base for the search//搜索域节点
            int totalResults = 0;
